...
const Chat = ({ route }) => {
  ...
  const receive_message_sound = new Sound(
    "receive_message.mp3",
    Sound.MAIN_BUNDLE
  );
  ...
  useEffect(() => {
    realm.write(() => {
      if (chatId) {
        const filteredChat = realm
          .objects("Message")
          .filtered("chat_id = $0", chatId);
  ...
  useEffect(() => {
    if (recipient) {
      const listener = supabase
        .channel("profilesChannel3")
        .on(
          "postgres_changes",
          { event: "*", schema: "*", table: "profiles" },
          (payload) => {
            if (payload.new.id === recipient.id) {
              ...
              realm.write(() => {
                const data = realm
                  .objects("Chat")
                  .filtered("recipient_id = $0", recipient.id);
                if (data.length > 0) {
  ...
  return (
    <SafeAreaView style={styles.container}>
            ...
            >
              <View style={styles.profilePic}>
                {recipient && recipient.profile_pic !== null ? (
                  <TouchableOpacity
                    onPress={() =>
                      navigation.navigate("recipientInfos", {
                        user: recipient,
                        id: chatId,
                      })
                    }
                  >
                    <Animated.Image
                      entering={FadeIn}
                      source={{ uri: recipient.profile_pic }}
                      style={{
                        height: "100%",
                        width: "100%",
                        borderRadius: 100,
                  ...
                  >
                    <Animated.Image
                      entering={FadeIn}
                      source={{ uri: route.params.user.profile_pic }}
                      style={{
                        height: "100%",
                        width: "100%",
                        borderRadius: 100,
                  ...
                  <Animated.Image
                    entering={FadeIn}
                    source={Assets.images.user_blank}
                ...
                {recipient &&
                  ...
                  isConnected && (
                    <Animated.View
                      entering={FadeIn}
                      style={styles.recipientStatusIndicator}
                    />
                  )}
              </View>
              {recipient && (
      ...
      <ImageBackground
        source={Assets.images.background_light}
...
const Row = ({
  ...
  const isSelected =
    selectedRow.length > 0 && selectedRow.some((prev) => prev.id === item.id);
  ...
  return (
    ...
    >
      ...
      <Animated.View
        entering={canAnimateBubble && FadeInDown}
        layout={Layout.stiffness(5)}
        style={[
          styles.bubble,
          isUser
            ? {
      ...
      >
        {replyToItem && item.status !== "deleted" && (
          <Animated.View
            entering={FadeIn}
            style={{
              backgroundColor: isUser ? "#7373EEBA" : "#f2f2f2",
              paddingVertical: 6,
            ...
            <View>
              <Text
                style={{
                  fontFamily: "Inter-Medium",
                  fontSize: 13,
                  color: isUser ? "white" : Assets.colors.black,
                }}
              >
                {replyToItem.sender_username === user.id
                  ? "Me:"
                  : replyToItem.sender_username}
              </Text>
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                {replyToItem.attachments &&
                  replyToItem.attachments.length > 0 && (
        ...
        <Text
          style={{
            ...
            fontSize: 14,
            color: isUser ? "white" : Assets.colors.black,
            fontStyle: item.status === "deleted" ? "italic" : "normal",
          }}
        >
          {item.content}
        </Text>
...
const RowMedia = ({ item, height, width, isUser }) => {
  ...
  return extension === "jpg" || extension === "png" || extension === "jpeg" ? (
    <TouchableOpacity
      onPress={() =>
        navigation.navigate("mediaViewer", { medias: item, type: "single" })
      }
    ...
    >
      ...
      {loading && (
        <ActivityIndicator
          size="small"
          color={isUser ? "white" : Assets.colors.background}
...
const ActionBar = ({
      ...
      if (canSend) {
        try {
          ...
          if (!chatId) {
            ...
            newChatId = `chat-${currentDate.getFullYear()}-${
              currentDate.getMonth() + 1
            }-${currentDate.getDate()}_${currentDate.getHours()}-${currentDate.getMinutes()}-${currentDate.getSeconds()}_${
              user.id
            }-${recipient.id}`;
          ...
          const message_id = `${currentDate.getFullYear()}-${
            currentDate.getMonth() + 1
          }-${currentDate.getDate()}_${currentDate.getHours()}-${currentDate.getMinutes()}-${currentDate.getSeconds()}_${
            user.id
          }-${recipient.id}`;
          ...
          if (attachments.length > 0) {
            let arr = [...attachments];
            let uploadedMedia = await Promise.all(
              arr.map(async (item) => {
                delete item.id;
                const saveMedia = await supabase.storage
                  .from("chat")
                  .upload(`${newChatId}/${item.name}`, item, {
                    cacheControl: "3600",
                    upsert: false,
                ...
                if (saveMedia.error) {
                  ...
                  if (saveMedia.data) {
                    const {
                      ...
                      .from("chat")
                      .getPublicUrl(`${newChatId}/${item.name}`);
                    if (publicUrl) {
                      return publicUrl;
                    }
                    return null;
          ...
          const createMessage = await supabase
            .from("messages")
            .insert(newMessage);
  ...
  const openGallery = async () => {
    try {
      const currentDate = new Date();
      const random_id = `${currentDate.getFullYear()}-${
        currentDate.getMonth() + 1
      }-${currentDate.getDate()}_${currentDate.getHours()}-${currentDate.getMinutes()}-${currentDate.getSeconds()}_${
        user.id
      }-${recipient.id}`;
      ...
      if (response && response.length > 0) {
        ...
        response.forEach((item) => {
          let media = {
            ...
            name: "Chat-" + random_id + item.fileName,
            type: item.mime,
          };
          arr.push(media);
          id++;
    ...
    <Animated.View style={styles.actionBar}>
      ...
      <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
        ...
        <Button
          text={
            !loading ? (
              <Assets.icons.Send variant="Bold" color="white" size={25} />
            ) : (
      ...
      {attachments.length > 0 && (
        ...
        >
          <Button
            ...
            size={30}
            onPress={() =>
              navigation.navigate("chatMediaViewer", {
        ...
        >
          <View style={styles.replyTextContainer}>
            ...
            <View style={{ maxWidth: "50%" }}>
              ...
              >
                {replyTo.sender_id === user.id ? "Me:" : recipient.username}
              </Text>
